...
from util.analysis import (calc_tran_activation, oap_peak_calc, diast_ind_calc,
                           ...
                           calc_snr,draw_mask,cond_vel,mult_cond_vel)
...
class MainWindow(QWidget, Ui_MainWindow):
    ...
    def Cbar_lim(self):
           limit2 = float(self.Cbar_max.text())
           limit1 = float(self.Cbar_min.text())
    ...
    def draw_mask_area(self): 
        ...
        tt=gg.astype(int)
        img_mod = cv2.fillPoly(self.data_prop[10,:,:], pts= [tt], color = (0, 0, 0))
        ...
        savetxt('mask_area.csv', mask_area, delimiter=',')
    ...
    def use_saved_mask_area(self):
        ...
        mask_area = loadtxt('mask_area.csv', delimiter=',')
        f = plt.figure()
        sp = f.add_subplot(111)
    ...
    def single_vect_cv (self):
        ...
        #for i in range(no_v):
        pts2=cond_vel(self.act_val,no_v)
        act=(self.act_val)
        plt.close('all')
        ff = plt.figure()
        c_fig = ff.add_subplot(111)
        c_fig.imshow(self.act_val)

        for i in range(no_v):
            plt.arrow(x=pts2[2*i,0],y=pts2[2*i,1],dx=(pts2[2*i+1,0]-pts2[2*i,0]),dy=(pts2[2*i+1,1]-pts2[2*i,1]),width=0.04,head_width=3,head_length=3) 
            dxx=(abs((pts2[2*i+1,0]+pts2[2*i,0]))/2).astype(int)-3
            dyy=(abs((pts2[2*i+1,1]+pts2[2*i,1]))/2).astype(int)+3
            vl=np.sqrt(abs((pts2[2*i+1,0]-pts2[2*i,0])**2+(pts2[2*i+1,1]-pts2[2*i,1])**2)).astype(int)
            vl=vl/scale_factor
            pt1=self.act_val[pts2[2*i,1],pts2[2*i,0]]
            pt2=self.act_val[pts2[2*i+1,1],pts2[2*i+1,0]]
            if abs(pt2-pt1) != 0: 
               self.velocity=(vl/abs(pt2-pt1)).astype(int)  
            else: 
               self.velocity=0
            plt.annotate(self.velocity,xy=(dxx,dyy), ha='center', va='center', size=11, color='red')
        return ff
       
    def multi_vect_cv(self):
        scale_factor=float(self.image_scale_edit.text())
        pt=mult_cond_vel(self.act_val)
        plt.close('all')
        ...
        y_sz=np.shape(self.act_val)[0]
        x_sz=np.shape(self.act_val)[1]
        ...
        for i in range(no_v):
             vl=np.sqrt(abs((pos[i,0]-pt[0,0])**2+(pos[i,1]-pt[0,1])**2)).astype(int)
             vl=vl/scale_factor
             yy=pos[i,1].astype(int)
             xx=pos[i,0].astype(int)
             print(xx)
             print(yy)
             pt2=self.act_val[yy,xx]
             pt1=self.act_val[pt[0,1],pt[0,0]] 
             if pt2-pt1 > 0: 
        ...
        for i in range(no_v): 
           ...
           dy=pos[i,1]-pt[0,1] 
           if velocity[i,0] > 0: 
               plt.arrow(x=pt[0,0],y=pt[0,1],dx=0.6*dx*velocity[i,0],dy=0.6*dy*velocity[i,0],width=0.04,head_width=2,head_length=2)
     ...
     for i in range(row-1):
          for  j in range(col-1):
              if i < row-1 and i > 0:
                  if j < col-1 and j > 0:
                      if ind[i,j]==0:
                          plt.arrow(x=j,y=i,dx=0,dy=velocity[i,j],width=0.0005,head_width=0.1,head_length=0.1)
                      if ind[i,j]==1:
                          plt.arrow(x=j,y=i,dx=velocity[i,j],dy=velocity[i,j],width=0.0005,head_width=0.1,head_length=0.1)
                      if ind[i,j]==2:
                          plt.arrow(x=j,y=i,dx=velocity[i,j],dy=0,width=0.0005,head_width=0.1,head_length=0.1)  
                      if ind[i,j]==3:
                         plt.arrow(x=j,y=i,dx=velocity[i,j],dy=-velocity[i,j],width=0.0005,head_width=0.1,head_length=0.1)
                      if ind[i,j]==4:
                         plt.arrow(x=j,y=i,dx=0,dy=-velocity[i,j],width=0.0005,head_width=0.1,head_length=0.1)
    ...
    def prep_data(self, progress_callback):
        ...
        if self.normalize_checkbox.isChecked():
            ...
            for i in range(aa[1]):
                for j in range(aa[2]): 
                    self.data_filt[:,i,j]=(self.data_filt[:,i,j]-np.amin(self.data_filt[:,i,j]))\
                    /(np.amax(self.data_filt[:,i,j])-np.amin(self.data_filt[:,i,j]))
    ...
    def analysis_select(self):
        if self.analysis_drop.currentIndex() == 0:
            ...
            #place holder for running alternan map at some point
            alternan = 5
        elif self.analysis_drop.currentIndex() == 4:
            # Disable the APD tools
            self.max_apd_label.setEnabled(False)
            self.max_apd_edit.setEnabled(False)
            self.max_apd_edit.setText('')
        ...
        elif self.analysis_drop.currentIndex() == 5:
            # Enable the APD tools
            self.max_apd_label.setEnabled(False)
            self.max_apd_edit.setEnabled(False)
            self.perc_apd_label_01.setEnabled(True)
    ...
    def map_analysis(self, progress_callback):
        ...
        if analysis_type == 2:
            ...
            self.act_ind = calc_tran_activation(
                self.data_filt, start_ind, end_ind)
            imaging_analysis.apd_80_analysis(self.data_filt, li1, li2, transp,
                                              start_ind, end_ind)
    ...
    def map_analysis_cbar(self, progress_callback):
        ...
        # Generate action repolarization map
        if analysis_type == 5:
